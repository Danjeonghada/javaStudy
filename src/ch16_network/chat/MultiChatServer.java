	...
	public void serverStart() {
		...
		try {
			...
			while(true) {
				...
				clientsList.add(client);  // 해당 서버 참여 접속자
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	// 여러 접속 클라이언트에게 메시지를 전해 주기 위한 클래스
	public class Client extends Thread{
		...
		public void sendAll(String msg) {
			for(int i = 0; i < clientsList.size(); i++) {
				clientsList.get(i).writer.println(msg);
				clientsList.get(i).writer.flush();
			}
		}
		@Override
		public void run() {
